...
/*   Updated: 2025/10/14 14:55:19 by aadeikal         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	write_char_to_buffer(char c, t_buffer *s_buffer)
{
    if(!s_buffer)
		return ;
	if (s_buffer->buffer && *(s_buffer->pos) < s_buffer->max_size - 1)
...
{
    if (!s_buffer->buffer || s_buffer->max_size == 0)
        return;
    
    if (*(s_buffer->pos) < s_buffer->max_size)
